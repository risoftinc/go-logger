...
func customRequestIDExample() {
	...
	ctx1 := logger.WithRequestID(context.Background(), "req-123")
	...
	config2 := logger.LoggerConfig{
		OutputMode:   logger.OutputTerminal,
		LogLevel:     logger.LevelInfo,
		LogDir:       "logs",
		RequestIDKey: "trace_id",
	}
	log2 := logger.NewLoggerWithConfig(config2)
	defer log2.Close()

	ctx2 := logger.WithRequestID(context.Background(), "trace-456")
	...
	config3 := logger.LoggerConfig{
		OutputMode:   logger.OutputTerminal,
		LogLevel:     logger.LevelInfo,
		LogDir:       "logs",
		RequestIDKey: "correlation_id",
	}
	log3 := logger.NewLoggerWithConfig(config3)
	defer log3.Close()

	ctx3 := logger.WithRequestID(context.Background(), "corr-789")
	...
	config4 := logger.LoggerConfig{
		OutputMode:   logger.OutputTerminal,
		LogLevel:     logger.LevelInfo,
		LogDir:       "logs",
		RequestIDKey: "session_id",
	}
	log4 := logger.NewLoggerWithConfig(config4)
	defer log4.Close()

	ctx4 := logger.WithRequestID(context.Background(), "sess-abc123")
	...
	// API Service logging
	apiCtx := logger.WithRequestID(context.Background(), "api-req-001")
	...
	bgCtx := logger.WithRequestID(context.Background(), "job-bg-002")
	...
	config6 := logger.LoggerConfig{
		OutputMode:   logger.OutputTerminal,
		LogLevel:     logger.LevelInfo,
		RequestIDKey: "", // Empty key means no request ID logging
	}
	log6 := logger.NewLoggerWithConfig(config6)
	defer log6.Close()

	ctx6 := logger.WithRequestID(context.Background(), "req-999")
